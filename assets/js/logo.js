...
function init() {
  ...
  var tween = TweenMax.fromTo(textAnimation, 4,
    {animationProgress:0},
    {animationProgress:1, ease:Power1.easeInOut, repeat:0, yoyo:true}
  );
...
// loader.load( './fonts/helvetiker_regular.typeface.json', function(font){
    var geometry = generateTextGeometry('C O M I N G  S O O N', {
        size:12,
        height:0,
        font:'droid sans',
        weight:'bold',
...
function TextAnimation(textGeometry) {
  ...
  for (i = 0, i2 = 0, i3 = 0, i4 = 0; i < faceCount; i++, i2 += 6, i3 += 9, i4 += 12) {
    ...
    var c0y = THREE.Math.randFloat(50, 120) * dirY;
    var c0z = THREE.Math.randFloat(-20, 20);

    var c1x = THREE.Math.randFloat(50, 60) * dirX;
    var c1y = THREE.Math.randFloat(0, 60) * dirY;
    var c1z = THREE.Math.randFloat(-60, 20);

    for (v = 0; v < 9; v += 3) {
      aControl0.array[i3 + v    ] = c0x;
      aControl0.array[i3 + v + 20] = c0y;
      aControl0.array[i3 + v + 20] = c0z;
